...
BEGIN;
DECLARE
    cursor_prod_back CURSOR FOR
                        select * from products;
FETCH LAST FROM cursor_prod_back;
MOVE BACKWARD 4 FROM cursor_prod_back;
FETCH PRIOR FROM cursor_prod_back;
MOVE BACKWARD 7 FROM cursor_prod_back;
FETCH PRIOR FROM cursor_prod_back;
MOVE BACKWARD 4 FROM cursor_prod_back;
FETCH PRIOR FROM cursor_prod_back;
FETCH PRIOR FROM cursor_prod_back;
CLOSE cursor_prod_back;
COMMIT;